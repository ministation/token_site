...
async def startup():
    ...
    # Инициализация SQLite для соцсети
...
# Вход через Discord (стандартный OAuth2)
@app.get("/login")
async def login():
...
# Вход по UUID (как в игре) – используется для привязки игрового аккаунта к существующей сессии
@app.get("/login/{user_uuid}")
async def login_with_uuid(user_uuid: str, request: Request):
    # Проверяем, существует ли такой игрок в БД
    player = await find_player_by_uuid(user_uuid)
    if not player:
        raise HTTPException(status_code=404, detail="Игрок с таким UUID не найден")
    # Ищем активную сессию (если пользователь уже авторизован через Discord)
    token = request.cookies.get("session_token")
    if token and token in user_sessions:
        sess = user_sessions[token]
        sess['player'] = player
        # Создаём/обновляем профиль в соцсети
        import database_social as social_db
        social_db.get_or_create_social_user(player['player_id'], player['user_uuid'], sess['discord_id'], sess['username'], sess.get('avatar'), player['last_seen_user_name'])
        save_sessions(user_sessions)
        return RedirectResponse("/")
    # Если сессии нет, создаём временную сессию только с игроком (без Discord)
...
async def callback(code: str, state: str):
    ...
    # Пробуем автоматически привязать игрока, если Discord уже связан
