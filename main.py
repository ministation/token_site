...
app = FastAPI(title="SS14 Social", docs_url=None, redoc_url=None)
...
from models_social import (
    ProfileUpdate, PostCreate, CommentCreate
)
...
async def startup():
    ...
    social_db.init_db(SOCIAL_DB_PATH)
    print("✅ SQLite готов")
...
async def find_player_by_nick(nick: str):
    async with db_pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT player_id, user_id::text as user_uuid, last_seen_user_name FROM player WHERE LOWER(last_seen_user_name)=LOWER($1) LIMIT 1", nick
        )
...
def get_current_player(request: Request):
    ...
    if 'player' not in user:
        raise HTTPException(403, detail="Привяжите игровой аккаунт")
    return user['player']
...
async def callback(code: str, state: str):
    ...
    user_sessions[session_token] = {'discord_id': discord_id, 'username': username, 'avatar': avatar_url, 'is_admin': discord_id in ADMIN_DISCORD_IDS, 'created': datetime.datetime.now().isoformat()}
...
async def api_me(request: Request):
    ...
    return {"authenticated": True, "username": sess['username'], "avatar": sess.get('avatar'), "player": sess.get('player'), "is_admin": sess.get('is_admin', False)}
...
async def post_chat(request: Request, msg: ChatMessage):
    user = get_current_user(request)
    chat_messages.append({"username": user['username'], "avatar": user.get('avatar'), "message": msg.message, "timestamp": datetime.datetime.now().isoformat()})
    if len(chat_messages) > MAX_CHAT_MESSAGES:
        chat_messages.pop(0)
    return {"success": True}

# -------------------- СОЦСЕТЬ (ПРОФИЛИ, ПОСТЫ, ПОДПИСКИ, ЛИЧНЫЕ СООБЩЕНИЯ) --------------------
...
async def send_message(request: Request, req: PrivateMessageRequest):
    ...
    if not req.message.strip():
        raise HTTPException(400)
    social_db.send_private_message(player['player_id'], req.receiver_player_id, req.message.strip())
    return {"success": True}
