...
function renderBanCard(b) {
    ...
    return '<div style="border-left:4px solid ' + colors[b.type] + '; margin-bottom:14px; padding:14px; background:rgba(25,18,40,0.7); border-radius:8px;">' +
        ...
        '<div><b>Админ:</b> ' + (b.admin_name || '-') + ' | <b>Игроки:</b> ' + players + ' | <b>Срок:</b> ' + exp + '</div>' +
        (roles ? '<div style="margin-top:6px;"><b>Роли:</b> ' + roles + '</div>' : '') +
        '<div style="background:rgba(0,0,0,0.3); padding:10px; margin-top:8px; border-radius:6px;"><b>Причина:</b> ' + (b.reason || '-') + '</div></div>';
}
