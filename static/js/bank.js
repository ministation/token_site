...
function stopSlotAnimation(prize) {
    ...
    document.querySelectorAll('.slot-reel').forEach(r => r.classList.remove('slot-spinning'));
    let symbols;
    if (prize >= 13) symbols = [SLOT_SYMBOLS[4], SLOT_SYMBOLS[4], SLOT_SYMBOLS[4]];
    else if (prize >= 7) symbols = [SLOT_SYMBOLS[3], SLOT_SYMBOLS[3], SLOT_SYMBOLS[1]];
...
function showEconomyTab(tab, btn) {
    ...
    const tabMap = { wallet: 'economyWallet', lottery: 'economyLottery' };
