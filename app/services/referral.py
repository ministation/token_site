...
async def apply_referral_code(referred_account_id: str, code: str) -> tuple[bool, str]:
    code = (code or "").strip().upper()
    if not code:
        return False, "Введите реферальный код"

    referred = social_db.get_social_user_by_discord_id(referred_account_id)
    if not referred:
        return False, "Сначала войдите на сайт"
    ...
    if referrer["discord_id"] == referred_account_id:
        return False, "Нельзя использовать свой код"

    if not social_db.mark_referral_used(referred_account_id, code, referrer["discord_id"]):
        return False, "Не удалось применить код"

    await _grant_coins(referrer["discord_id"], REFERRAL_REFERRER_COINS, "referrer")
    await _grant_coins(referred_account_id, REFERRAL_REFEREE_COINS, "referee")
...
def get_referral_info(account_id: str) -> dict:
    user = social_db.get_social_user_by_discord_id(account_id)
    if not user:
        return {}
    code = user.get("referral_code") or ensure_referral_code(account_id)
    stats = social_db.get_referral_stats(account_id)
    return {
        "code": code,
        "referrals_count": stats.get("count", 0),
        "referred_by": user.get("referred_by_code"),
        "needs_prompt": social_db.needs_referral_prompt(account_id),
