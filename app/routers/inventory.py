...
async def api_inventory(request: Request):
    ...
    if user.get("player"):
        ...
        ckey = user["player"].get("last_seen_user_name")
    ...
    if not ckey:
        ckey = user["social"].get("game_nickname") or user.get("username")

    return await get_inventory(user["discord_id"], user_uuid, ckey)
